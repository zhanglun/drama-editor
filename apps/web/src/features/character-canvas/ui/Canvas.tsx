...
interface HandleMenuState {
  x: number; y: number
  nodeId: string; nodeType: 'character' | 'variant'
  mode: 'click' | 'drag'
  canvasPosition: { x: number; y: number }
}

function CanvasInner({
  scriptId, onEditVariant, onAddVariant, onNodeSelect, onConnectVariant, onCreateCharacter,
}: CanvasProps) {
  ...
  const { handleNodeDragStop } = useNodeDrag({ updateNodePosition })
  ...
  const handleNodeContextMenu = (event: React.MouseEvent, node: Node) => {
    ...
    setContextMenu({ x: event.clientX, y: event.clientY, nodeId: node.id, nodeType: node.type as 'character' | 'variant' })
  }
  ...
  const handleDeleteNode = useCallback(
    (nodeId: string) => {
      if (confirm('确定要删除该节点吗？')) removeNode(nodeId)
    },
    [removeNode],
  )
  ...
  const handleHandleInteraction = (
    ...
    const canvasPosition = screenToFlowPosition({
      x: interaction.position.x - bounds.left, y: interaction.position.y - bounds.top,
    })
    setHandleMenu({ x: interaction.position.x, y: interaction.position.y, nodeId, nodeType, mode: interaction.mode, canvasPosition })
  }

  const handleMenuSelect = (action: string) => {
    if (action === 'create-variant' && handleMenu) onAddVariant?.(handleMenu.nodeId, handleMenu.nodeType)
    setHandleMenu(null)
  }
  ...
  const nodesWithCallbacks = nodes.map((node) => ({
    ...
    data: {
      ...node.data,
      onHandleInteraction: (interaction: { mode: 'click' | 'drag'; position: { x: number; y: number } }) =>
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center"><p className="text-sm text-red-500">{error}</p></div>
      </div>
    )
  }
  ...
  return (
    <div className="h-full w-full relative" ref={reactFlowWrapper}>
      ...
      >
        ...
        <MiniMap nodeStrokeColor="#e2e8f0" nodeColor={(node) => node.type === 'character' ? '#6366f1' : '#8b5cf6'} maskColor="rgba(0,0,0,0.1)" />
      </ReactFlow>

      {selectedNodeId && (
        <CanvasToolbar nodeId={selectedNodeId}
      ...
      {contextMenu && (
        <CanvasContextMenu x={contextMenu.x} y={contextMenu.y} nodeId={contextMenu.nodeId} nodeType={contextMenu.nodeType}
          onEdit={() => { if (contextMenu.nodeId) onEditVariant?.(contextMenu.nodeId); setContextMenu(null) }}
          onDelete={() => { if (contextMenu.nodeId) handleDeleteNode(contextMenu.nodeId); setContextMenu(null) }}
          onAddChild={() => { if (contextMenu.nodeId) onAddVariant?.(contextMenu.nodeId, contextMenu.nodeType || 'variant'); setContextMenu(null) }}
          onCreateCharacter={() => { onCreateCharacter?.(); setContextMenu(null) }}
          onClose={() => setContextMenu(null)}
        />
      )}

      {handleMenu && (
        <HandleMenu x={handleMenu.x} y={handleMenu.y} nodeId={handleMenu.nodeId} nodeType={handleMenu.nodeType}
          mode={handleMenu.mode} onSelect={handleMenuSelect} onClose={() => setHandleMenu(null)}
        />
      )}
    </div>
  )
