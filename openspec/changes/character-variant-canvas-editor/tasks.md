...
- [x] 9.1 创建 `apps/web/src/pages/CharacterCanvasPage.tsx` 页面组件，布局画布和侧边栏
- [x] 9.2 添加路由配置：`/scripts/:id/characters/canvas`
- [x] 9.3 在现有角色列表页面添加"画布视图"切换按钮
- [x] 9.4 实现画布视图和列表视图的双向切换

## 10. 前端 - 形象编辑面板

- [x] 10.1 创建形象详情侧边栏面板：显示/编辑名称、描述、图片 URL、颜色、特征属性
- [x] 10.2 创建场景关联面板：展示已关联场景列表、添加/移除关联
- [x] 10.3 创建特征属性编辑器：支持动态添加/编辑/删除键值对
- [x] 10.4 实现从形象跳转到剧本编辑器对应场景的功能

## 11. 集成测试

- [ ] 11.1 验证数据库迁移执行和回滚
- [x] 11.2 验证后端 API 端点的 CRUD 操作
- [x] 11.3 验证形象层级关系的创建、查询和删除
- [x] 11.4 验证形象-场景关联的创建和删除
- [x] 11.5 验证画布状态的保存和恢复
